...
const Color _kUnselected = Color(0xFFE5E5E5);
const Color _kTopBorder = Color(0xFF2A2A2A);

class BarberScaffold extends StatelessWidget {
  const BarberScaffold({required this.navigationShell, super.key});
...
class _BarberBottomNav extends StatelessWidget {
  const _BarberBottomNav({required this.currentIndex, required this.onTap});
        ...
        child: SizedBox(
          ...
          child: Row(
            children: List.generate(_tabs.length, (idx) {
              final (IconData icon, String label) = _tabs[idx];
