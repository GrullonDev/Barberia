...
class _ConfirmationPageState extends ConsumerState<ConfirmationPage> {
                      ...
                      Padding(
                        ...
                        child: Column(
                          children: <Widget>[
                            ...
                            Text(
                              tr.confirm_code_suffix('Cita Confirmada'),
                            ...
                            const SizedBox(height: 20),
                            _buildQrSection(qrData, tr, context),
                          ],
                        ),
                      ),
                      _buildFooterActions(context, booking, tr),
                    ],
                  ),
                ),
                const SizedBox(height: 20),
                _buildSecondaryActions(context, tr),
              ],
            ),
          ),
        ),
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        Wrap(
          ...
          runSpacing: 4,
          children: <Widget>[
            _InfoChip(icon: Icons.person, label: booking.customerName),
            if (booking.customerPhone != null)
              _InfoChip(icon: Icons.phone, label: booking.customerPhone!),
            if (booking.customerEmail != null)
              _InfoChip(icon: Icons.email, label: booking.customerEmail!),
  ...
  Widget _buildQrSection(String qrData, S tr, BuildContext context) {
    return Column(
      children: <Widget>[
        GestureDetector(
          onLongPress: () async {
            await Share.share(qrData, subject: 'Ubicación Barbería');
          },
          child: Center(
            child: Semantics(
        ...
        const SizedBox(height: 8),
        Text(
          tr.confirm_scan_qr_for_directions,
              ...
              onPressed: () async {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(tr.confirm_link_copied)),
                  );
                }
              },
            ),
  ...
  Widget _buildFooterActions(BuildContext context, Booking booking, S tr) {
      ...
      child: Row(
        ...
        children: <Widget>[
          TextButton.icon(
            onPressed: () {
              ...
              final Event event = Event(
                title: tr.confirm_add_calendar_subject,
          ...
          TextButton.icon(
            ...
            label: Text(tr.confirm_get_directions),
          ),
        ],
      ),
    );
  ...
  Widget _buildSecondaryActions(BuildContext context, S tr) {
    return Row(
      children: <Widget>[
        Expanded(
